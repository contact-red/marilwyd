...
class val _Unrecognized
  ...
  call `whoami` when it restores a session. Its first request can reach an
  endpoint marilwyd does not implement. If that request answered a flat 404,
  a client holding a token from before a restart would be told "no such
  endpoint" and never "your session is gone", so it would keep the stale
  token, keep showing a signed-in account, and wait for data that cannot
  arrive.
