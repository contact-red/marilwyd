...
class \nodoc\ iso _TestUnimplementedRejectsStaleToken is UnitTest
  """
  Element does not call whoami when it restores a session. Its first request
  can reach an endpoint marilwyd does not implement. A stale token must still
  be reported before the missing endpoint.
  ...
  fun apply(h: TestHelper) =>
    _Serve(
      ...
      {(r) =>
        ...
        _AssertJSONBool(h, r, "soft_logout", true)
      } val)
